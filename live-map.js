  ...
  */
  ...
  async function loadMetro(){
    ...
    stopLines.forEach((lines,id)=>{
      ...
      const html=`<div class="metro-stop-v10 ${used?'used':''}">
        <span class="metro-symbol">M</span>
        <span class="metro-lines">${badges}</span>
      </div>`;
      const icon=L.divIcon({className:'',html,iconSize:[used?68:58,28],iconAnchor:[14,14]});
  ...
  Promise.allSettled([loadMetro(),loadFood(),loadPlannedFoodReal()]).then(async results=>{
    ...
    el.innerHTML=m?'✓ Metro reale caricata'+(f?' · fast food caricati':'')+(p?' · ristoranti pianificati geolocalizzati':''):'⚠ Metro live non disponibile';
